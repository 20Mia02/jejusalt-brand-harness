  ...
  return {
    brand: {
      ...
      characterStyleGuideline:
        "모든 캐릭터는 귀엽고 매력적인 인형(마스코트) 같은 스타일 - 동글동글한 외형, 부드러운 색감, 친근한 표정을 기본으로 한다. 실존하는 유명 캐릭터를 연상시키는 디자인은 절대 금지",
      characterCommonMotif:
        "알(egg) 모양의 동글동글한 몸통, 이마 위 작은 육각형 소금 결정 브로치(제주소금 시그니처), 단순한 검은 점 눈동자에 흰색 하이라이트 하나, 은은한 볼터치 - 이 요소들은 모든 캐릭터가 공통으로 가져서 같은 브랜드임을 알아볼 수 있게 한다",
