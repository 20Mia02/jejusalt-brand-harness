...
⚠️ 매우 중요한 판정 원칙: "인형같은 귀여운 외모"는 좋지만, 실제로 촬영된 물리적
인형/피규어 사진(사실적인 원단 짜임, 플라스틱/비닐 광택 반사, 스튜디오 제품사진 조명,
경직되고 생명력 없는 느낌)처럼 보이면 안 됩니다. 애니메이션/일러스트 캐릭터 렌더처럼
따뜻하고 살아있는 느낌이어야 합니다. 실제 사진처럼 보일수록 cutenessScore와
detailScore를 감점하세요 (이건 "디테일이 많다"고 가점 요소가 아니라 감점 요소입니다).

1. cutenessScore (0-100, 50% 비중, 최우선): 눈(크고 살아있는 느낌, 30점) + 얼굴 비율(둥근
   얼굴/큰 눈/작은 입/부드러운 곡선, 15점) + 표정(따뜻하고 다가가고 싶은 미소, 15점) +
   전체 매력(애니메이션 캐릭터처럼 사랑스럽고 생명력 있는 느낌, 실제 사진 같으면 감점, 20점) +
   색상(따뜻하고 부드러운 톤, 10점) + 포즈(자연스럽고 사랑스러움, 10점)
2. detailScore (0-100, 30% 비중): 액세서리/의상/특징이 얼마나 선명하게 "그려져" 있는가
   (단, 귀여움을 해치지 않는 범위 내에서, 그리고 사실적인 사진 재질로 보이지 않는 범위 내에서).
   텍스트/로고/글자가 원치 않게 들어갔으면 감점. 실사 제품사진처럼 보이면 감점.
...
async function refineCharacterImage({ characterConfig, maxRetries = MAX_RETRIES }) {
  let currentPrompt = characterConfig.higgsfieldPromptTemplate;
  const attempts = [];
