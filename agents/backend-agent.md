...
- 각 Agent 호출 결과를 검증
- Higgsfield 영상 생성 요청 및 진행률 폴링 (videos 테이블 5초마다 UPDATE)
...
### 3-1. 함수 목적
Higgsfield API를 호출하여 영상 생성 요청 + videos 테이블에 INSERT (진행 중)
...
resourceId: UUID (generation_logs 연결용)
contentId: UUID (videos의 content_id FK)
```
...
#### ⭐ 핵심: videos 테이블에 INSERT (진행 중에!)
```
INSERT INTO videos (
  resource_id,
  content_id,                    ← FK 추가!
  higgsfield_id,                 ← Higgsfield 반환값
  generation_status: "processing", ← ⭐ 진행 중 표시
  generation_progress: 0,        ← ⭐ 0%로 시작
  generation_start_time: NOW()
)
```

**중요**: 이 row는 pollHiggsfield()에서 5초마다 업데이트됨
...
성공 시:
{
  ...
  data: {
    ...
    generation_status: "processing",      // ⭐ 추가
    generation_progress: 0,               // ⭐ 추가
    content_id: contentId                 // ⭐ 추가
  }
}
...
### 4-1. 함수 목적
Higgsfield 영상 생성 진행률을 모니터링하고, **videos 테이블 업데이트** (진행률 추적!)
...
| **callHiggsfield** | videoConfig | Higgsfield 호출 + videos INSERT (진행 중!) | {success, higgsfieldId, generation_status} |
| **pollHiggsfield** | higgsfieldId | 5초마다 진행률 조회 + videos UPDATE | {success, progress, status, videoUrl} |
...
- **Higgsfield 폴링: 진행 중에도 videos row 존재 (5초마다 UPDATE)**
- 폴링 최대 시간: 10분
